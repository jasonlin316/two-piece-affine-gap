...
reg tb_valid, array_num;
reg [`DIRECTION_WIDTH*`N-1:0] row_k0, row_k1;
//outputs
wire [`SEQUENCE_ELEMENT_WIDTH-1:0] query_alignment_out, target_alignment_out;
...
traceback DUT(.clk(clk), .max_position_x(max_position_mem[0]), .max_position_y(max_position_mem[1]), 
			  .prefetch_row(prefetch_row), .sequence_in(sequence_in),
			  .query_alignment_out(query_alignment_out), .target_alignment_out(target_alignment_out), .alignment_valid(alignment_valid),
...
always @(negedge clk) begin
	if (alignment_valid)begin
		S_alignment_out[l] = query_alignment_out;
		T_alignment_out[l] = target_alignment_out;
		l = l+1;
	end
end
...
begin
    for(m=0;m<l;m=m+1)begin
        if(S_alignment_out[m] !== S_alignment_golden[l-m-1]) begin
         	$display("ERROR at S_alignment_out[%d]: output %d != expect %d ",m, S_alignment_out[m], S_alignment_golden[l-m-1]);
         	err = err+1 ;
		end
	...
	if (err === 0 &&  over===1'b1  )  begin
	            ...
	            for(i=0; i<l; i=i+1)begin
	            	$write("%d, ", S_alignment_out[i]);
	            end
	            $display("\n");
	            for(i=0; i<l; i=i+1)begin
	            	$write("%d, ", T_alignment_out[i]);
	            end
	            $display("-------------------PASS-------------------\n");
		    #(t_reset) $finish;
	         end
